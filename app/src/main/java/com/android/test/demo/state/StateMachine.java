 ...
 *
 * 这里定义的状态机是一种层次状态机，它处理消息，并可以将状态分层排列。
 *
 ...
 *
 * 一个状态是一个State类型的对象，它必须实现processMessage()，可选实现enter()/exit()/getName()。
 * enter()/exit()方法与面向对象编程中的构造和注销类似，用于对状态进行初始化和清理。
 * getName方法返回的是默认返回State的类名。
 *
 ...
 *
 * 当一个状态机被创建的时候，addState()被用来构建层次结构，而setInitialState()被用来识别哪一个是初始状态。
 * 构建之后调用start()来初始化并启动状态机。状态机的第一个动作是从树的根节点开始调用enter()。
 * enter()在状态机处理消息之前调用。
 *
<code>
        mP1
 ...
 *
 * 状态机在创建并开始后，通过sendMessage()向状态机发送消息（通过obtainMessage()构建消息）。
 * 当状态机收到消息时，当前的State的processMessage()将会被调用。
 *
 ...
 *
 * 每一个State有0个或者１个父State，如果子State不能处理消息时（processMessage()返回了false或者NOT_HANDLED），
 * 消息将会转发给父State处理，如果消息一直为被处理则unhandledMessage()将会被调用
 *
 *
 ...
 *
 * 当所有处理都完成后，可以选择调用状态机的transitionToHaltingState()接口,
 * 在当前processingMessage返回时，状态机将转换到内部的HaltingState状态，并调用halting()。
 * 状态机接下来接收的任何消息都将导致调用haltedProcessMessage()。
 *
 *
 ...
 * 如果希望完全停止状态机，可以调用quit()或者quitNow(),这时当前状态以及父状态都会进入exit(),
 * 然后执行onQuiting()并退出Thread/Looper。
 *
 *
 * <p>In addition to <code>processMessage</code> each <code>State</code> has
 * an <code>enter</code> method and <code>exit</exit> method which may be overridden.</p>
 * 另外，除了processMessage()外，enter()和exit()都可能被重写
 *
 ...
 *
 * 状态的另外两个可用方法deferMessage()和sendMessageAtFrontOfQueue(): sendMessageAtFrontOfQueue()在
 * 队列的最前面发送一条消息,而不是后面。deferMessage()是将该消息被保存在一个列表中，直到转向一个新状态
 * 后才开始处理。在转换后，所有延迟消息都将放在状态机消息队列的前面。这些消息将在新状态队列的其他消息之前处
 * 理。这两种方法都是受保护的，并且只能从状态机中调用。
 *
        ...
        private void performTransitions(State msgProcessedState, Message msg) {
            ...
            if (destState != null) {
                ...
                while (true) {
                     ...
                     * message queue.
                     * 翻译：由于我们已经过渡到一个新的状态，所以我们需要将任何延迟消息移动到消息队列
                     * 的前面，以便在消息队列中的任何其他消息之前处理它们。
                     */
